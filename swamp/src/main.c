...
int agregarPaginaDinamica(int pid, int pagina, int contenido){
...
void cambiarOrdenPorBorado( Carpincho_Swamp* car1){
    ...
    for (int i = 0;i<max;i++){
        ...
        //printf("EL pid es %d \n",car->pid);
        if(car->pid == car1->pid){
            list_remove(carpinchosEnArchivo, i);
            i=max;
        }
    }
    
    int ordenProximo;
    max = list_size(carpinchosEnArchivo);
    for (int i = 0;i<max;i++){
