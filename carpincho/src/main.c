...
void *carpincho(void *config)
{
  ...
  printf("estrara en while\n");
  int i =0;
  while (i < 5)
  {
    sem_wait(info->producer_sem);
    printf("paso el wait del orto\n");
 ...
 puts("calculo con lo que le llego");
...
int main(int argc, char *argv[])
{
  logger = log_create(LOG_PATH, PROGRAM_NAME, 1, 2);
...
/*    int main(int argc, char *argv[])
{
 ...
 printf("creo sems\n");
    pthread_create(&carpincho1_thread, NULL, (void *)exec_carpincho_1, "cfg/carpincho.config");
     printf("creo hilo1\n");
    pthread_create(&carpincho2_thread, NULL, (void *)exec_carpincho_2, 
