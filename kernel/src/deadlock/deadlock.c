...
{   if(terminar){
    ...
    log_info(logger_deadlock, "Comienza la ejecución de la detección de Deadlock");
    if (!list_is_empty(lista_sem_kernel))
    {
        for (int i = 0; i < list_size(lista_sem_kernel); i++)
        {
            log_info(logger_deadlock, "Entro al for deadlock");
        ...
        log_info(logger_deadlock, "Finaliza la búsqueda de carpinchos en posible deadlock");
        ...
        if (!list_is_empty(lista_posible_deadlock))
        {
            ...
            log_info(logger_deadlock, "Verificando espera circular");
            ...
            log_info(logger_deadlock, "Destruyendo lista de posible deadlock");
            if(list_is_empty(lista_posible_deadlock))
        ...
        else
        {
            ...
            log_info(logger_deadlock, "No se encontraron carpinchos en posible deadlock");
            log_info(logger_deadlock, "Destruyendo lista de posible deadlock vacia");
...
void lista_deadlock_destroyer(void *arg)
{
    ...
    log_info(logger_deadlock, "Liberando carpincho %d involucrado en deadlock", a_destruir->pid);
...
t_list *verificar_espera_circular(t_list *lista)
{
    ...
    if (cerrado)
    {
       // printf("ENCONTRO CIRCULOOOOOOO DE %d ELEMENTOS\n", list_size(lista_aux));
        log_info(logger_deadlock, "Espera circular detectada. Cantidad de carpinchos en deadlock: %d",list_size(lista_aux));
        return lista_aux;
    }
    else
    {
        ...
        log_info(logger_deadlock, "No se detectó espera circular");
        return NULL;
    }
}
//--------------------------------------------------------------
   ...
   // bool encontrado = false;
    ...
    while (i < list_size(lista_deadlock) )
    {
        ...
        if (mayor_id < aux->pid)
        {
            ...
            log_info(logger_deadlock, "Se finaliza el carpincho PID %d", mayor_id);
            int pos;
...
void sacar_de_cola_bloqueados(sem_kernel *semaforo, int id)
{
    ...
    while (i < list_size(semaforo->bloqueados) && !removido)
    {
        ...
        if (carpincho->pid == id)
        {
            ...
            sem_post(&semaforo->mutex_cola);
            log_info(logger_deadlock, "carpincho %d con deadlock removido de lista\n", carpincho->pid);
